...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    data_loss = np.sum(-1 * np.log(normalized_p[range(num_train), y])) / num_train
    ...
    grad_scores = normalized_p
    grad_scores[range(num_train), y] -= 1
    grad_scores /= num_train
    # Backpropagate scores gradient to gradient w.r.t W2
    grads['W2'] = np.dot(relu_h.T, grad_scores) + (2 * reg * W2)
    grads['b2'] = np.sum(grad_scores, axis=0)
    # Backpropagate scores gradient to gradient w.r.t W2
    grad_hidden = np.dot(grad_scores, W2.T)
    # Take gradient of relu
    grad_hidden[relu_h <= 0] = 0
  ...
  def predict(self, X):
    ...
    relu_h = np.maximum(0, h)
    scores = relu_h.dot(self.params['W2']) + self.params['b2']
